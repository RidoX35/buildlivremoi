...
const createProduct = async (req, res) => {
  ...
  // Récupérer tous les produits par l'id de la companie
  const getAllProduct = async (req, res) => {
    ...
    const getAllProductByName = async (req, res) => {
      try {
        const products = await itemProduct.find({companieId: req.params.name});
      
        // Vérifier si des produits ont été trouvés
        if (products.length === 0) {
          return res.status(404).json({ message: "Aucun produit trouvé pour cette companieName." });
        }
  ...
  module.exports = { createProduct, getAllProduct, getProductById, updateProduct, deleteProduct, getAllProductByName };
